...
.. tip::
    ...
    In this example, we create an ``RssBlock``. An RSS block already exists in
    the BlockBundle, but this one is more powerful as it allows to define a
...
the location where the RSS feed should be shown. The easiest is to extend
``Symfony\Cmf\Bundle\BlockBundle\Doctrine\Phpcr\AbstractBlock`` but you are
free to do your own document. At least, you have to implement
...
already existing block service. For our RSS block we need a custom a service
that knows how to fetch the feed data of an ``RssBlocks``::
...
a template. See :ref:`bundle-block-execute` for more on the block service
``execute`` method. To not make your page very slow by fetching the feed
on each request, have a look at the :doc:`cache` documentation.
